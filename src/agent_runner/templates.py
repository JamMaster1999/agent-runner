...
# RUNNER_JOB_KEY      — the runner's tracking identity for this work; bound to
#                       the pipeline run id until extraction replaces run ids.
#                       Fills _meta.run_id and the embedded job_event.py
#                       progress command's --run-id.
...
#                       the primary output and every phase-3/4 sibling
#                       artifact, which a single-file variable could not.
# RESOURCE:cdp_browser.* — attributes of the declared phase-3 CDP browser
#                       resource (D4). Substitution values are JSON-encoded
#                       scalars, so one template renders a quoted string when
#                       the orchestrator supplies a browser and null when the
#                       agent manages its own.
