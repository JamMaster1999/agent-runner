...
- **Stall watchdog**: alive is not the same as working — and silent is not the same as dead. The watchdog accepts three proofs of life: stream output, process-tree CPU growth (`/proc`, Linux), and workdir file activity. A CLI silent on every channel for fifteen minutes is reaped and the attempt ends `stalled`, so a wedged agent is retried in a quarter hour instead of sitting out the run's multi-hour backstop — while an agent quietly running a long command can never be mistaken for one. `AGENT_RUNNER_STALL_SECONDS` tunes the window; only caller code can switch the watchdog off (`Policy(stall_seconds=0)`) — a zero, negative, or unreadable environment value falls back to the default.
