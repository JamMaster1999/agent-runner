...
Several accounts of one CLI form a **pool** (`agent_runner.pool.Pool(var, credentials, share)`), handed to `run_sandboxed_attempt(pool=...)`. Each attempt runs on the least-loaded account with room under its cap and waits, heartbeating, while every account is full; `share` (your worker's slots divided among the accounts) is the most one account is ever asked to carry. A rate-limited attempt says which limit it hit: `rate` (too many requests at once) halves the account's cap and re-runs the attempt in place after a jittered pause, `server` re-runs it after the pause, `usage` (the subscription window is spent) holds the account until the reset the CLI named and lets the activity retry on the next free account. Every valid attempt grows its account's cap back by one.
